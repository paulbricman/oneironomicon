...
class QuestionAnsweringAssistance():
    ...
    def compute_reward(self, dialog_history, qa_threshold=0.7, sim_threshold=0.5):
        ...
        past_qaness = [db_qaness.get(e) for e in student_replies[1:-1]]
        current_qaness = self.qa_model.predict(
            [(question, student_replies[-1])])[0]
        db_qaness[student_replies[-1]] = current_qaness
        good_student_replies = [e for e_idx, e in enumerate(
            student_replies[1:-1]) if past_qaness[e_idx] > qa_threshold]
        ...
        current_emb = self.encoder_model.encode(
            student_replies[-1], convert_to_tensor=True)
        ...
        highest_similarity = util.semantic_search(
            current_emb, past_embs)[0][0]['score']
